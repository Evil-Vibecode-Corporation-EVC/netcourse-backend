...
Получить общее количество посещений сайта.
...
### POST /site-visits/increment

Увеличить счетчик посещений сайта на 1.
...
Создать или продлить подписку для текущего пользователя (заглушка, без реальной оплаты).
...
**Response (продление существующей):** `200`
- Если у пользователя уже есть активная подписка, срок `expiresAt` продлевается на выбранный план (оставшееся время + новый период).
...
Отменить свою активную подписку (устанавливает `status = "cancelled"`).

**Аутентификация:** требуется.

**Response (подписка найдена):**
```json
{
  "id": 1,
  "userId": 1,
